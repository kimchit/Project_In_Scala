...
object TokenizingFunctions extends App{
  ...
  lazy val KeyWordsPattern= List("class","constructor","function","method","file","static","var","int","char","boolean","void","true","false","null","this","let","do","if","else","while","return")
  ...
  def  Q2(line:String,tokenString:String):Unit={ //number
    ...
    val cur_char=line(0)
    cur_char match{
      case NumberPattern(cur_char)=> //number
          tokenString.appended(cur_char)
          Q2(line.substring(1),tokenString)
          Breaks
      case _=>//anything else means that a integer token is read
          GenTokenInteger(tokenString)
          Q0(line)
    }
